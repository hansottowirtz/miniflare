...
// const MIN_MULTIPART_UPLOAD_SIZE = 5 * 1024 * 1024; // TODO: switch back, or make configurable
const MIN_MULTIPART_UPLOAD_SIZE = 50;
...
function assembleMultipartValue(
  storage: NewStorage,
  parts: Pick<MultipartPartRow, "blob_id" | "size">[],
  queryRange: InclusiveRange
): ReadableStream<Uint8Array> {
  // Stream required parts (and the ranges within them) to satisfy the query
  const identity = new TransformStream<Uint8Array, Uint8Array>();
  let start = 0;
  // The `Promise` returned from `pipeTo()` won't resolve until a reader starts
  // reading, so run this in the background as an async IIFE.
  (async () => {
    for (const part of parts) {
      ...
      if (rangeOverlaps(partRange, queryRange)) {
        ...
        const value = await storage.blob.get(part.blob_id, range);
        // TODO(now): figure out what to do in this case, likely referenced
        //  part deleted while reading, and already serving back to `workerd`
        assert(value !== null);
  ...
  })().catch((e) => identity.writable.abort(e));
  ...
  return {
    ...
    completeMultipartUpload: db.transaction(
      (key: string, upload_id: string, selectedParts: R2PublishedPart[]) => {
        ...
        if (uploadRow?.state === MultipartUploadState.COMPLETED) {
          throw new NoSuchUpload();
        ...
        for (const part of parts.slice(0, -1)) {
          if (part.size < MIN_MULTIPART_UPLOAD_SIZE) {
        ...
        for (const part of parts.slice(0, -1)) {
          ...
          if (part.size < MIN_MULTIPART_UPLOAD_SIZE || part.size !== partSize) {
            throw new BadUpload();
          }
        }
