...
export type Ranges =
  | InclusiveRange
  | InclusiveRange[]
  | /* `Range` header value */ string;

function stringifyInclusiveRange({ start, end }: InclusiveRange) {
  return `[${start},${end}]`;
}
function stringifyRange(range: Ranges) {
  if (Array.isArray(range)) {
    return range.map(stringifyInclusiveRange);
  } else if (typeof range === "object") {
    return stringifyInclusiveRange(range);
  } else {
    return JSON.stringify(range);
  }
}
export class RangeNotSatisfiableError extends MiniflareError {
  constructor(readonly totalSize: number, readonly range: Ranges) {
    super("ERR_RANGE", `Invalid range: ${stringifyRange(range)}`);
  }
}
 ...
 * @internal
 */
export function _parseRangeHeader(
  size: number,
  rangeHeader: string
  ...
  for (const range of ranges) {
    ...
    if (start !== undefined && end !== undefined) {
      ...
      if (rangeStart >= size) return; // Start after content (Range Not Satisfiable)
      if (rangeEnd >= size) rangeEnd = size - 1;
      result.push({ start: rangeStart, end: rangeEnd });
    } else if (start !== undefined && end === undefined) {
      const rangeStart = parseInt(start);
      if (rangeStart >= size) return; // Start after content (Range Not Satisfiable)
      result.push({ start: rangeStart, end: size - 1 });
    } else if (start === undefined && end !== undefined) {
      const suffix = parseInt(end);
      if (suffix >= size) return []; // Entire Response
      if (suffix === 0) continue; // Empty range
      result.push({ start: size - suffix, end: size - 1 });
    } else {
...
function parseRange(
  size: number,
  { start, end }: InclusiveRange
): InclusiveRange {
  if (0 <= start && start <= end && start < size) {
    if (end >= size) end = size - 1;
    return { start, end };
  } else {
    throw new RangeNotSatisfiableError(size, { start, end });
  }
}

/** @internal */
export function _parseRanges(size: number, ranges?: Ranges): InclusiveRange[] {
  if (Array.isArray(ranges)) {
    return ranges.map((range) => parseRange(size, range));
  } else if (typeof ranges === "string") {
    const parsed = _parseRangeHeader(size, ranges);
    if (parsed === undefined) throw new RangeNotSatisfiableError(size, ranges);
    return parsed;
  } else if (typeof ranges === "object") {
    return [parseRange(size, ranges)];
  } else {
    return [];
  }
}
